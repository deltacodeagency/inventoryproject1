...
export const ReportsView: React.FC = () => {
  ...
  const renderSubReport = () => {
    switch (activeView) {
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl space-y-1">
                <span className="text-[10px] text-slate-400 font-bold uppercase block">Total Cashier Invoices</span>
              ...
              <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl space-y-1">
                ...
                <p className="text-lg font-black text-slate-800">
                  ৳{sales.length > 0 ? (totalSalesSum / sales.length).toFixed(2) : '0.00'}
                </p>
              </div>
            </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl space-y-1">
                ...
                <p className="text-lg font-black text-slate-800">{purchases.length} Batches</p>
              </div>
            </div>
      ...
      // 8. ANNUAL REPORT
      case 'rep-annual':
        const totalCOGSVal = sales.reduce((sum, s) => sum + (s.costOfGoodsSold !== undefined ? s.costOfGoodsSold : s.items.reduce((iSum, item) => iSum + (item.cost || 0) * item.quantity, 0)), 0);
